...
class CArray {
	...
	/**
	 *
	 * Searching into associative array by presented keys
	 *
	 * $key can be:
	 *      - string, if is string you should devide into parts (keys) using delimeter ','
	 *      - array, list of keys for search
	 *
	 * @example
	 *      $arr = array(
	 *          'nest' => array(
	 ...
	 *          )
	 *      );
	 *      assocSearch('nest, inner, value', $arr); // output: 12, using string
	 *      assocSearch(array('nest', 'inner', 'value'), $arr); // output: 12, using array
	 *
	 * @param mixed $keys
	 * @param array $searchArray
	 * @param bool $exceptionOnError if is true then if key not exists into array will throws CException, else return $defaultReturn
	 * @param null $defaultReturn
	 *
	 * @throws CException
	 * @return array|null
	 */
	public static function assocSearch($keys, array $searchArray, $exceptionOnError = true, $defaultReturn = null) {
		if (is_string($keys)) {
		...
		foreach ($keys as $key) {
			if (!array_key_exists($key, $searchArray)) {
				if ($exceptionOnError) {
					throw new CException(Yii::t('app', 'Property \':key\' not exists into source array', array(':key' => $key)));
				}
	...
	public static function export($array) {
		...
		return preg_replace("/\t{1,} array/", " array", $string) . ';';
	}
