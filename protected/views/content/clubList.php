...
<div class="main_nav fl_l">
    <h5>Фитнес клубы</h5>
    <?php $this->widget('application.widgets.ClubMenuWidget',array(
        'active' => $category,
    )); ?>
        ...
        <div class="week_best">
            ...
            <ul class="week_best_list">
                <?php foreach ($lastNews as $item): ?>
                    <li>
                        <span class="category_name"><a href="<?php echo $item->club->getUrlAbout(); ?>"><?php echo $item->club->club; ?></a></span>
                        <a href="<?php echo $item->getUrl($item->club->slug); ?>" class="description"><?php echo $item->title; ?></a>
                    </li>
                <?php endforeach; ?>
            </ul>
        </div>
...
<ul class="main_content_list clubs">
    <?php foreach($pickedClubs as $elem): ?>
    <li class="vip">
        <div class="img_cont with_preview fl_l">
            <a class="img_lg_box" href="<?php echo Yii::app()->createUrl('club/'.$elem->slug.'/about'); ?>">
                <?php foreach($elem->images as $innerKey => $innerElem){
                    if($innerKey == 5) break;
                    echo '<img class="img_lg img_lg'.$innerKey.'" width="200" height="149" src="/pub/club/photo/200x149/'.$innerElem['image_filename'].'" alt="1164389797_1384907646918995.jpg">';
                } ?>
            </a>
            <ul class="img_preview_box">
                <?php foreach($elem->images as $innerKey => $innerElem){
                    if($innerKey == 5) break;
                    echo '<li>'.CHtml::link('<img data-img-class="img_lg'.$innerKey.'" width="38" height="39" src="/pub/club/photo/38x39/'.$innerElem['image_filename'].'" alt="1164389797_1384907646918995.jpg">','/club/'.$elem->slug.'/about.html').'</li>';                } ?>
            </ul>
        </div>
        ...
        <div class="item_info">
            ...
            <div class="club_service">
                <ul>
                    <?php foreach($elem->props as $innerElem): ?>
                        <li><?php echo Yii::app()->image->getImageTag($innerElem->image_id,30,30); ?></li>
                    <?php endforeach; ?>
                </ul>
            </div>
        </div>
