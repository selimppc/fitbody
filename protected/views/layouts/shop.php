<?php $this->beginContent('//layouts/main'); ?>
    ...
    <div class="main clearfix inner_page_single">
        <div class="clubs_inner compact">
            ...
            <h1><?php echo FunctionHelper::upperFirst($this->shop->title); ?></h1>
            <div class="img_cont fl_l">
                <?php if(isset($this->shop->images[0])) echo Yii::app()->image->getImageTag($this->shop->images[0]->id,130,90); ?>
            </div>

            <div class="clubs_inner_info">
                <div class="address">г.<?php echo FunctionHelper::upperFirst($this->shop->addressesRel[0]->city->city); ?>,<?php echo $this->shop->addressesRel[0]->address; ?> <div class="map_link"><?php echo CHtml::link("на карте",'/shop/'.$this->shop->slug.'/about.html#show_map'); ?></div></div>
                <div class="work_time">
                    <dl>
                        <?php foreach($this->shop->addressesRel[0]->worktimes as $key => $elem): ?>
                            <dt><?php echo $this->days[$elem->from_day].($elem->from_day == $elem->to_day ? '' : ' - '.$this->days[$elem->to_day].': '); ?></dt>
                            <dd class="clearfix">c <?php echo substr($elem->from_time, 0, 5); ?> до <?php echo substr($elem->to_time, 0, 5); ?><?php echo (isset($this->shop->addressesRel[0]->worktimes[$key+1]) ? ',' : '');?></dd>
                        <?php endforeach; ?>
                    </dl>
                </div>
                <div class="phone">
                    <?php foreach($this->shop->addressesRel[0]->phones as $elem): ?>
                        <span><?php echo $elem->phone; ?><?php if($elem->description != '') echo ' <span class="phone_comment">('.$elem->description.')</span>'; ?></span>
                    <?php endforeach; ?>
                </div>
                <div class="site"><a href="http://<?php echo $this->shop->site; ?>"><?php echo $this->shop->site; ?></a></div>
            </div>
        </div>
