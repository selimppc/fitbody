...
include_once __DIR__ . '/Transliteration.php';
...
class SluggableBehavior extends CActiveRecordBehavior
{
    ...
    public function beforeSave($event)
    {
        ...
        if ($this->translit === true) {
            $this->translator = new Transliteration();
            $string = $this->translator->transliterate($string);
        }

        // First version of slug
        if (true === $this->useInflector) {
            $slug = $checkslug = Doctrine_Inflector::urlize($string);
        } else {
            $slug = $checkslug = $this->simpleSlug($string);
        }
