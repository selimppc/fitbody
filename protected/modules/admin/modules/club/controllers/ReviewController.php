...
class ReviewController extends AdminController {
    ...
    public function actionUpdate() {
        ...
        if (Yii::app()->request->isAjaxRequest && isset($_POST['pk'])) {
            ...
            $es = new EditableSaver('ClubReview');
            $es->update();
            Yii::app()->end();
        }
        ...
        if ($id = Yii::app()->request->getParam('id')) {
            $model = ClubReview::model()->findByPk($id);
            ...
            $model = new ClubReview();
        }

        $model->setScenario('adminScenario');
        $materials = Club::model()->findAll();
        $materials = CHtml::listData($materials, 'id', 'club');
        ...
        if ($post = Yii::app()->request->getPost('ClubReview')) {
            $model->attributes = $post;
            $model->type = ClubReview::TYPE_OF_REVIEW;
            if ($model->save()) {
                $this->redirect($uri);
            }
