...
class UploadAction extends CAction {
    ...
    protected function handleDeleting($imageId) {
        if ($imageId) {
            ...
            Yii::app()->image->delete($Image->id);
//            if (!$imageSizes = ImageSize::model()->findAll('image_object_id = :image_object_id', array(':image_object_id' => $Image->image_object_id))) {
//                echo json_encode(true);
//                return true;
//            }
//
...
//            Image::model()->deleteByPk($imageId);
            TempImage::deleteImages($imageId);
        ...
        if ($model->{$this->fileAttribute} !== null) {
            ...
            if ($model->validate()) {
                ...
                if ($image->save()) {
                    ...
                    Yii::log("UploadAction: " . 'fileUpload', CLogger::LEVEL_ERROR, "UploadAction");
                }
            } else {
