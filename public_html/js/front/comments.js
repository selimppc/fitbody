var comments = {
    ...
    toggleForm: function(e) {
        ...
        if ($target.attr('data-reply') === 'open') {
            ...
            var commentForm = $(tpl).attr('action', location.pathname + '#comment-id-' + parentId).wrap('<li></li>').parent().addClass('comment_edit').find('input.comment_parent_id').first().attr('id', 'Comment' + parentId).val(parentId).closest('li.comment_edit');
