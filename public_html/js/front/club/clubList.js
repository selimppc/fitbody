...
$(function(){
    ...
    var destination_id = window.destination_id;

    var getClubs = new request('application.controllers.ClubController','getNearestClubs').listen(function(response){
        if(response){
    ...
    var onButton = function(coords){
        $.ajax({
            "type"      : "post",
            "url"       : '/ajax-request/club/get-clubs',
            "data"      : {data : {coords : coords, range : map.range.val(), destination_id : destination_id}},
            "dataType"  : "json",
...
//        getClubs.data({coords : coords, range : map.range.val(), destination_id : destination_id}).send();
    };
